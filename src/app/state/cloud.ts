...
class Cloud {
  ...
  delta(state:Object, TWEEN_:any, csphere:THREE.Mesh, callback:Function){
    ...
    mediator.logc(`cloud.delta: state['_cloud'] = ${state['_cloud']}`);
    if(_cloud === undefined || _cloud === true){  // undefined/true
      ...
      try {
          ...
          textureLoader.load(urls[i],
            ...
            (texture) => {
              ...
              for (let j=0; j<particlesByN; j++ ) {
                ...
                objects.push(spr);
                group.add(spr);
                mediator.log(`spritecloud positions i=${i} j=${j}`);
              }
              if(loaded === N){
            ...
            // progress
            (xhr) => {
              mediator.log(`cloud loading textures...`);
            },
            // error
            (xhr) => {
        ...
        mediator.loge(`error in spritecloud_init: ${e.message}`);
        callback(null, null);
      }
    }else{  // _cloud-false so remove
