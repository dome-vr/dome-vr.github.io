...
// ===== NEW: Sunset miss color function =====
vec3 sunsetMissColor(vec3 rd){
  // TEMPORARY: Very obvious color for testing - BRIGHT RED/ORANGE
  // Once confirmed working, we can tone it down
  
  // First check if we have a skybox texture to sample from
  if(uSkyMode == 3){
    // Sample from the pisa-sunset cube texture
    vec3 skyboxColor = texture(uSkyCube, rd).rgb;
    // Mix with warm tint to make it obvious it's working
    return mix(skyboxColor, vec3(0.8, 0.3, 0.1), 0.3);
  }
  
  // Fallback: VERY OBVIOUS sunset gradient based on ray direction
  float upness = rd.y * 0.5 + 0.5;  // 0 to 1, based on looking up or down
  
  // BRIGHT sunset colors for testing
  vec3 horizonColor = vec3(0.9, 0.4, 0.2);  // BRIGHT warm orange
  vec3 zenithColor = vec3(0.4, 0.2, 0.3);   // Purple-pink
  
  return mix(horizonColor, zenithColor, upness);
}
...
// REMOVED: uniform float uRed;
 ...
 float _distance(vec3 p, vec3 v, vec3 b){
   ...
   // Slightly less rounding to make geometry more solid
   return length(max(abs(q)-b, 0.0)) - 0.2;  // Reduced from 0.25 to 0.2
...
vec4 DomeOriginalShading(vec3 ro, vec3 rd, float t, vec3 n, vec3 baseCol){
  ...
  // ===== NEW: Gradient control variable =====
  bool useGradient = false;  // Set to true to enable complex patterns
  
  if (useGradient) {
    // Complex pattern rendering path (original behavior)
    if (uColorCycleActive) {
    ...
    // Simple gradient path (basic spatial variation but no complex patterns)
    if (uColorCycleActive) {
      // Simple spatial gradient with color cycling
      float timeShift = iTime * uColorCycleSpeed;
      
      // Simple spatial gradient based on position
      vec3 gradientBase = vec3(0.6, 0.4, 0.8) + 0.2 * normalize(hitPoint);
      
      vec3 colorShift = uColorCycleAmplitude * cos(
        6.28318 * (
          uColorCycleOffset +              // User-configurable palette offset
          uColorCyclePhase * timeShift     // Pure time-based cycling
        )
      );
      
      baseColor = gradientBase + colorShift;
    } else {
      // Simple spatial gradient - no cycling
      // Creates a gentle gradient based on world position
      baseColor = vec3(0.6, 0.4, 0.8) + 0.2 * normalize(hitPoint);
    }
  }
...
bool march(vec3 ro, vec3 rd, out float t, out vec3 hp){
  ...
  for(int i=0;i<MAX_MARCH_STEPS;i++){
    vec3 p = ro+rd*t;
    // REMOVED early return on far distance to give sunset color a chance
    // if(length(p)>800.0) return false;
    float d = Dist(p);
    if(abs(d)<float(HIT_EPSILON)){ hp=p; return true; }
    
    // More conservative stepping to prevent holes in small geometry
    float stepSize = min(d * 0.8, 0.1); // Limit max step size and use smaller multiplier
    t += stepSize;
    
    if(t>float(FAR_PLANE)) break;
    if(length(p)>800.0) break;  // Break instead of return to reach sunset color
  }
  return false;
}
...
void main(){
  ...
  if(uDebugMode==1){
    ...
    if(hit){
      ...
      // MODIFIED: Use sunset miss color instead of skyColor
      col = sunsetMissColor(rd);
    }
  }
  if(uDebugStereo){
