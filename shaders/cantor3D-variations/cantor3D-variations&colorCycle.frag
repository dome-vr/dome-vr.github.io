...
const vec3 cameraPos = vec3(0,0,1);
const vec3 cameraLookat = vec3(0,0,-1);
...
const float HOLE_SIZE = 1.5;     // 1.0 = original size, 0.5 = smaller holes, 1.5 = larger holes
const int HOLE_SHAPE = 1;        // 0=square, 1=circle, 2=triangle, 3=hexagon
...
// Original square cross function
float CrossSquare(vec3 p, float holeSize)
{
   p = abs(p);
   vec3 d = vec3(max(p.x, p.y),
                 max(p.y, p.z),
                 max(p.z, p.x));
   return min(d.x, min(d.y, d.z)) - (holeSize / 3.0);
}

// Circular holes version
float CrossCircle(vec3 p, float holeSize)
{
   p = abs(p);
   // Create circular holes by using cylindrical distances
   float dx = length(p.yz) - holeSize/3.0;
   float dy = length(p.xz) - holeSize/3.0; 
   float dz = length(p.xy) - holeSize/3.0;
   return min(dx, min(dy, dz));
}

// Triangular holes version
float CrossTriangle(vec3 p, float holeSize)
{
   p = abs(p);
   float s = holeSize / 3.0;
   
   // Create triangular holes using distance to triangle edges
   float dx = max(p.y + p.z * 0.577 - s * 1.155, max(p.y - p.z * 0.577 - s * 0.577, p.z - s));
   float dy = max(p.x + p.z * 0.577 - s * 1.155, max(p.x - p.z * 0.577 - s * 0.577, p.z - s));
   float dz = max(p.x + p.y * 0.577 - s * 1.155, max(p.x - p.y * 0.577 - s * 0.577, p.y - s));
   
   return min(dx, min(dy, dz));
}

// Hexagonal holes version
float CrossHexagon(vec3 p, float holeSize)
{
   p = abs(p);
   float s = holeSize / 3.0;
   
   // Hexagonal distance function
   vec2 h = vec2(s, s * 0.866); // hex proportions
   
   float dx = max(abs(p.y) - h.y, max(abs(p.z) * h.x + p.y * h.y, abs(p.z) * h.x - p.y * h.y) - h.x * h.y);
   float dy = max(abs(p.x) - h.y, max(abs(p.z) * h.x + p.x * h.y, abs(p.z) * h.x - p.x * h.y) - h.x * h.y);
   float dz = max(abs(p.x) - h.y, max(abs(p.y) * h.x + p.x * h.y, abs(p.y) * h.x - p.x * h.y) - h.x * h.y);
   
   return min(dx, min(dy, dz));
}

// Unified Cross function that switches between shapes
float Cross(vec3 p)
{
   if (HOLE_SHAPE == 1) {
      return CrossCircle(p, HOLE_SIZE);
   } else if (HOLE_SHAPE == 2) {
      return CrossTriangle(p, HOLE_SIZE);
   } else if (HOLE_SHAPE == 3) {
      return CrossHexagon(p, HOLE_SIZE);
   } else {
      return CrossSquare(p, HOLE_SIZE); // Default: square holes
   }
}
...
// Dome-sh dual rendering paths with FIXED color cycling support
vec4 DomeDefaultShading(vec3 ro, vec3 rd, float t, vec3 n, vec3 baseCol){
  ...
  if (uColorCycleActive) {
    ...
    // Apply offset OUTSIDE cosine to shift base palette
    vec3 staticColor = vec3(0.7, 0.75, 0.8) + uColorCycleOffset;  // Light blue-gray solid + offset
...
vec4 DomeOriginalShading(vec3 ro, vec3 rd, float t, vec3 n, vec3 baseCol){
  ...
  if (uColorCycleActive) {
    ...
    // Apply offset to base color OUTSIDE the cosine
    vec3 staticColor = vec3(0.5) + 0.3 * cos(6.28318 * (vec3(0.1, 0.2, 0.3) + vec3(0.4, 0.3, 0.2) * hitPoint));
    staticColor += uColorCycleOffset;  // Add offset to shift base palette
