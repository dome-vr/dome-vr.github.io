...
// ======= QUICK TOGGLES =======
#ifndef DEBUG_VIEW     // 0 = raymarch - show fractal3D infinite-pistons
#define DEBUG_VIEW 0   // 1 = show ray directions (color gradient) (no marching)
#endif
#ifndef TEST_GEOM      // guaranteed hit-sphere off
#define TEST_GEOM 0    // 1 = add a guaranteed-visible test sphere/plane
#endif
#ifndef SHOW_NORMALS   // 0 = no normals
#define SHOW_NORMALS 0 // 1 = visualize normals (color) on hits
#endif
...
Mat mapScene(vec3 p){
  ...
  // FIXED: Limit corridor depth to reduce horizon distortion
  float corridorDepth = 300.0; // Extended to accommodate 25 column segments
  ...
  vec3 rp = p;
  // Only repeat columns within reasonable distance (25 segments at 12.0 spacing = 300 units total)
...
// Sky modes: 0=black, 1=faceColors, 3=cubeTextures
