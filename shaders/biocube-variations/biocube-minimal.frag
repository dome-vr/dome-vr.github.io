...
vec3 faceColor(int fid){
  if(fid==0) return vec3(1.0,0.3,0.3);
  if(fid==1) return vec3(0.3,1.0,0.3);
  if(fid==2) return vec3(0.3,0.6,1.0);
  if(fid==3) return vec3(1.0,0.6,0.3);
  if(fid==4) return vec3(0.9,0.9,0.2);
  return vec3(0.6,0.3,1.0);
}
...
// Original lighting function - THIS IS THE KEY TO PRESERVING COLORS
vec3 lighting(vec3 p, vec3 n, vec3 rd) {
    vec3 lightPos = vec3(3.0, 4.0, 5.0);
    vec3 lightDir = normalize(lightPos - p);
    vec3 viewDir = -rd;
    
    // Material color based on position - THIS CREATES THE COLORFUL FRACTAL APPEARANCE
    vec3 baseColor = vec3(0.5) + 0.3 * cos(6.28318 * (vec3(0.1, 0.2, 0.3) + vec3(0.4, 0.3, 0.2) * p));
    
    // Add fractal iteration coloring - THIS ADDS DYNAMIC COLOR VARIATION
    float de = DE(p);
    baseColor *= (0.8 + 0.2 * sin(de * 20.0 + iTime));
    
    // Ambient
    vec3 ambient = vec3(0.05, 0.05, 0.1) * baseColor;
...
// Dome-sh compatible normal function
vec3 GetNormal(vec3 pos) {
  return calcNormal(pos);
}

// Dome-sh shading functions that preserve original colors
vec4 DomeDefaultShading(vec3 ro, vec3 rd, float t, vec3 n, vec3 baseCol){
  vec3 L = normalize(vec3(0.6,0.7,0.5));
  float diff = max(dot(n,L),0.0);
  vec3 lit = baseCol * (uAmbient + uHeadlight*diff);
  return vec4(lit,1.0);
}
...
vec4 DomeOriginalShading(vec3 ro, vec3 rd, float t, vec3 n, vec3 baseCol){
  ...
  // Use the ORIGINAL lighting function to preserve colors
  vec3 color = lighting(hitPoint, n, rd);
  
  // Add the original post-processing effects (toned down)
  // Glow effect (reduced intensity)
  float glow = 1.0 / (1.0 + t * t * 0.2);
  color += vec3(0.03, 0.02, 0.1) * glow;
  
  // Fog (more subtle)
  float fog = exp(-t * 0.05);
  color = mix(vec3(0.0, 0.0, 0.02), color, fog);
  
  // Gamma correction (less aggressive)
