...
export const Pointcloud = class {
    static create(options = {}) {
        ...
        const color = options['color'] || 'red', pointsize = options['pointsize'] || 2.0, radius = options['radius'] || 100, rotation_speed = options['rotation_speed'] || .005, transform = options['transform'], pcloud_g = new THREE.SphereBufferGeometry(radius, 160, 80), pcloud_m = new THREE.ShaderMaterial({
            uniforms: uniforms,
            vertexShader: vsh,
            fragmentShader: fsh,
            transparent: true
        }), pcloud = new THREE.Points(pcloud_g, pcloud_m), nvertices = pcloud_g.attributes.position.count, alphas = new Float32Array(nvertices * 1), psizes = new Float32Array(nvertices * 1);
...
//# sourceMappingURL=pointcloud - Copy.js.map